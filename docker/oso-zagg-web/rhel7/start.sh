...
/usr/local/bin/ops-run-in-loop 10 "/usr/bin/flock -n /var/tmp/ops-zagg-metric-processor.lock -c '/usr/bin/timeout -s9 600s /usr/bin/ops-zagg-metric-processor' " &
echo "Done."

# Process heartbeats every minute
echo -n "Starting heartbeat processing loop... "
/usr/local/bin/ops-run-in-loop 10 "/usr/bin/flock -n /var/tmp/ops-zagg-heartbeat-processor.lock -c '/usr/bin/timeout -s9 600s /usr/bin/ops-zagg-heartbeat-processor' "  &
echo "Done."
