...
'''
[[Ansible-Roles-SHOULD-use-lib-roles-for-functionality-sharing]]
[cols="2v,v"]
|===
| <<Ansible-Roles-SHOULD-use-lib-roles-for-functionality-sharing, Rule>>
| Ansible Roles SHOULD use lib_* roles for sharing functionality among multiple roles.
|===

Being able to share code among multiple Ansible roles is highly desired. The lib_* pattern is a pattern that allows for code resusability without needing to worry about tightly coupling functionality from one role to another.

.Lib_* roles:
* MUST NOT depend on other roles except possibly other lib_* roles.
* MUST NOT have a `tasks/main.yml` file (shouldn't execute any tasks by default).
* MAY include modules in a `library` directory.
* MAY include task files in `tasks` (include_role can execute these task files directly).

For roles, it is preferred to use loose coupling. Lib_* roles is one of the patterns that allows for this loose coupling.


'''
[[Ansible-Roles-MUST-NOT-depend-on-other-roles]]
[cols="2v,v"]
|===
| <<Ansible-Roles-MUST-NOT-depend-on-other-roles, Rule>>
| Ansible Roles MUST NOT depend on or call into other roles.
|===

The purpose of this rule is to make individual roles more easily callable. When roles depend on other roles, they are very hard to maintain as well as to satisfy all of the dependencies.

For roles, it is preferred to use loose coupling.

Roles that depend on other roles quickly create a situation where all dependencies of all roles must be met

.Exceptions:
* Tech profiles are allowed to depend on / call into other roles.
* All roles are allowed to depend on / call into lib_* roles.
...
=== Playbook flow and stages
'''
[[Playbooks-MUST-use-include_role]]
[cols="2v,v"]
|===
| <<Playbooks-MUST-use-include_role, Rule>>
| Playbooks MUST use include_role instead of roles:
|===

[[Playbooks-SHOULD-NOT-use-the-pre-roles-post-tasks-sections]]
[cols="2v,v"]
|===
| <<Playbooks-SHOULD-NOT-use-the-pre-roles-post-tasks-sections, Rule>>
| Playbooks SHOULD NOT use pre_tasks, post_tasks, and roles: sections.  Should use tasks: and include_role:.
|===

Playbook execution should be top-to-bottom.  The user should not be required to determine execution order of plays or sections.  This
is accomplished by removing the pre_tasks:, roles:, and post_tasks: sections in a playbook.  When a role is needed then include_role SHOULD be used instead.


include_role is the future of ansible.  It introduces a way to call roles that is flexible and allows for looping.  With the playbooks reading top-to-bottom the 
playbook execution is much easier to read and understand.
...
- hosts: somehost
  ...
  - name: required package
    package: 
      name: my_preinstall_requirement_package 
      state: latest"
    register: install_result
