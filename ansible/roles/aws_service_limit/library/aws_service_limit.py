...
def service_limits(ta):
    ...
    ta_query = ta.describe_trusted_advisor_checks(language='en')
    service_limits_id = None
    for el in ta_query['checks']:
        if el['name'] == "Service Limits":
            service_limits_id = el['id']

    # Gather results from Trusted Advisor
    ta_results = ta_conn.describe_trusted_advisor_check_result(checkId=service_limits_id)

    # extract limits from the most recent TA report
    service_limits = []
    for res in ta_results['result']['flaggedResources']:
        service_limits.append({ 'region' : res['metadata'][0],
