...
class SimpleZabbixRaw(object):
    ...
    def _run_ansible(self, args):
        ...
        # The leadup to the TaskQueueManager() call below is basically
        ...
        results = results_callback.result

        if results.is_unreachable():
            message = "Ansible module run failed: module output:\n%s" % \
                      json.dumps(results._result, indent=4)
            raise ResultsException(message)

        if results.is_failed():
            raise ResultsException(results._result)
